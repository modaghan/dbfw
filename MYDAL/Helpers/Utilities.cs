...
namespace BLL
{
    public static class Utilities
    {
        ...
        public static void SaveConnection(string Host, string UserId, string Password, string Catalog, string ConnectionTimeout)
        {
            string target = "Local";
            IniFile iniFile;
            ...
            iniFile.IniWriteValue(target, "Host", isCrypted ? hashCode.EncryptionConfig(Host) : Host);
            iniFile.IniWriteValue(target, "UserId", isCrypted ? hashCode.EncryptionConfig(UserId) : UserId);
            iniFile.IniWriteValue(target, "Password", isCrypted ? hashCode.EncryptionConfig(Password) : Password);
            iniFile.IniWriteValue(target, "Catalog", isCrypted ? hashCode.EncryptionConfig(Catalog) : Catalog);
            iniFile.IniWriteValue(target, "ConnectionTimeout", isCrypted ? hashCode.EncryptionConfig(ConnectionTimeout) : ConnectionTimeout);
        ...
        public static int Tamsayi(object sayi)
        {
            try
            {
                int i = 0;
        ...
        public static uint UTamsayi(object sayi)
        {
            try
            {
                uint i = 0;
                uint.TryParse(sayi.ToString(), out i);
                return i;
            }
            catch (Exception)
            {
